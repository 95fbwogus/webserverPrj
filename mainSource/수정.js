window.addEventListener('DOMContentLoaded', function() {
	...
	//기존 파일 개수 세기 및 유튜브 아이디 추출
	var orgYt = document.getElementById('org_yt');
	var orgCount = (document.getElementById('existing_files').childElementCount / 2);
	var ids2 = [];

	if (orgYt !== undefined) {
		for (var i = 0; i < orgYt.childElementCount; i++) {
			ids2[i] = orgYt.children[i].textContent;
		}
	}

	//기존 파일 삭제
	var orgPreview = document.getElementsByClassName('org_preview')[0];
	var fileExtensions = ['mp3', 'wav', 'flac', 'jpg', 'gif', 'png', 'bmp', 'yt'];
	...
	for (var i = 0; i < delFromOrgFile.length; i++) {
		delFromOrgFile[i].addEventListener('click', function() {
			max = (--orgCount);
			if (this.previousSibling.textContent.split('.')[1] === 'yt') {//삭제 요청한 것인 유튜브 아이디이면
				for (var i = 0; i < orgYt.childElementCount; i++) {//해당 유튜브 아이디를 재등록할 수 있도록 중복처리에서 제외해야 한다.
					//밑에서 ids2가 갖고 있는 유튜브 아이디와 새로 등록하려는 유튜브 아이디를 비교하는데
					//ids2에서 삭제 요청한 유튜브의 아이디는 제거해줘야 한다. 
					if (ids2[i].includes(this.previousSibling.textContent.split('.')[0])) {
						ids2[i] = '';
					};
				}
			}
		...
		delAtPreview[i].addEventListener('click', function() {
			for (var k = 0; k < existingFilesController.childElementCount; k++) {
				...
				if (this.parentElement.className == target_name) {
					...
					if (existingFilesController.children[k].children[0].textContent.split('.')[1] === 'yt') {//삭제 요청한 것인 유튜브 아이디이면
						for (var i = 0; i < orgYt.childElementCount; i++) {//해당 유튜브 아이디를 재등록할 수 있도록 중복처리에서 제외해야 한다.
							//밑에서 ids2가 갖고 있는 유튜브 아이디와 새로 등록하려는 유튜브 아이디를 비교하는데
							//ids2에서 삭제 요청한 유튜브의 아이디는 제거해줘야 한다. 
							if (ids2[i].includes(existingFilesController.children[k].children[0].textContent.split('.')[0])) {
								ids2[i] = '';
							};
						}
					}
	...
	ytInputBtn.addEventListener('click', function() {
		...
		if (max <= 10) {// 이 조건문은 없애도 된다.
			...
			if (ytId != false) {
				if (ids.indexOf(ytId) == -1 && ids2.indexOf(ytId) == -1) {//ids2는 기존에 업로드되었던 유튜브 아이디
					//iframe 생성
					ids[th] = ytId;
					//유튜브 미리보기 삭제 도구 생성
